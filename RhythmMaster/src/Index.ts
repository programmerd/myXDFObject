class Index extends eui.Component implements eui.UIComponent {
	...
	protected childrenCreated(): void {
		...
		this.gameOver.addEventListener(egret.TouchEvent.TOUCH_TAP,()=>{this.isMusic=true;this.musicFun();this.start()}, this);
	...
	public animationFun(dbObj) {
		...
		armature.addEventListener(dragonBones.EventObject.COMPLETE, () => {
			...
			if (this.isPlayMusic) {
				...
				if (this.isPlayAnimate.length >= 4 && this.subNum.value <= 1) {
					this.subNum.value++;
					this.isPlayMusic = false;
					this.musicFun();
				} else if (this.isPlayAnimate.length >= 4 && this.subNum.value >= 2) {
					console.log("恭喜通关");
					this.isPlayMusic = false;
					this.musicFun();
